...
public class PdfReportService {
    ...
    public ByteArrayInputStream generarReporteInventario(List<Product> productos) throws IOException {
        ...
        try (PDDocument document = new PDDocument()) {
            ...
            try (PDPageContentStream cs = new PDPageContentStream(document, page)) {
                ...
                drawHeader(document, cs, "REPORTE DE VALORACIÓN DE INVENTARIO", config, y);
                y -= 80;
                ...
                for (Product p : productos) {
                    ...
                    writeText(cs, FONT_NORMAL, 9, MARGIN + 10, y, p.getNombre(), COLOR_PRIMARY);
                    writeText(cs, FONT_NORMAL, 9, MARGIN + colWidths[0] + 10, y, p.getCategoria(), COLOR_SECONDARY);
                    writeText(cs, FONT_NORMAL, 9, MARGIN + colWidths[0] + colWidths[1] + 10, y, String.valueOf(p.getStock()), COLOR_PRIMARY);
                    writeText(cs, FONT_NORMAL, 9, MARGIN + colWidths[0] + colWidths[1] + colWidths[2] + 10, y, nf.format(p.getPrecio()), COLOR_SECONDARY);
                    writeText(cs, FONT_BOLD, 9, MARGIN + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] + 10, y, nf.format(subtotal), COLOR_PRIMARY);
    ...
    public ByteArrayInputStream generarReporteVentas(List<Venta> ventas, String inicio, String fin) throws IOException {
        ...
        try (PDDocument document = new PDDocument()) {
            ...
            try (PDPageContentStream cs = new PDPageContentStream(document, page)) {
                ...
                drawHeader(document, cs, "REPORTE DE VENTAS", config, y);
                y -= 50;
                writeText(cs, FONT_BOLD, 10, MARGIN, y, "Periodo: del " + inicio + " al " + fin, COLOR_ACCENT);
                y -= 30;
                ...
                for (Venta v : ventas) {
                    ...
                    writeText(cs, FONT_NORMAL, 9, MARGIN + 10, y, "#" + v.getId() + (isAnulada ? " (A)" : ""), isAnulada ? Color.RED : COLOR_PRIMARY);
                    writeText(cs, FONT_NORMAL, 9, MARGIN + colWidths[0] + 10, y, v.getFechaVenta().toString(), COLOR_SECONDARY);
                    writeText(cs, FONT_NORMAL, 9, MARGIN + colWidths[0] + colWidths[1] + 10, y, v.getCliente() != null ? v.getCliente() : "Final", COLOR_PRIMARY);
                    writeText(cs, FONT_NORMAL, 9, MARGIN + colWidths[0] + colWidths[1] + colWidths[2] + 10, y, String.valueOf(v.getDetalles().size()), COLOR_SECONDARY);
                    writeText(cs, FONT_BOLD, 9, MARGIN + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] + 10, y, nf.format(v.getTotal()), COLOR_PRIMARY);
    ...
    private void drawHeader(PDDocument doc, PDPageContentStream cs, String title, Configuracion config, float y) throws IOException {
        if (config.getLogoBase64() != null && config.getLogoBase64().contains(",")) {
            try {
                ...
                cs.drawImage(pdImage, MARGIN, y - 10, 45, 45);
            } catch (Exception e) {}
        } else {
        ...
        writeText(cs, FONT_BOLD, 18, MARGIN + 55, y + 15, title, COLOR_PRIMARY);
        String empresa = config.getNombreEmpresa() != null ? config.getNombreEmpresa() : "SISGI";
        writeText(cs, FONT_NORMAL, 10, MARGIN + 55, y, empresa + " | NIT: " + (config.getNit() != null ? config.getNit() : "N/A"), COLOR_SECONDARY);

        String dateStr = LocalDate.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy"));
        writeTextRight(cs, FONT_NORMAL, 10, 555, y + 15, "Emitido: " + dateStr, COLOR_SECONDARY);
    }
    ...
    private void drawSummaryBox(PDPageContentStream cs, String label, String value, float y, float rightX) throws IOException {
        ...
        writeText(cs, FONT_NORMAL, 10, x + 10, y + 5, label, COLOR_SECONDARY);
        writeTextRight(cs, FONT_BOLD, 16, rightX - 10, y + 5, value, COLOR_ACCENT);
    }

    private void drawFooter(PDPageContentStream cs, PDPage page, Configuracion config) throws IOException {
        cs.setNonStrokingColor(COLOR_SECONDARY);
        String footer = "SISGI Inventory Management System | " + (config.getNombreEmpresa() != null ? config.getNombreEmpresa() : "Gestión de Inventarios");
        float w = FONT_NORMAL.getStringWidth(footer) / 1000 * 8;
        writeText(cs, FONT_NORMAL, 8, (page.getMediaBox().getWidth() - w) / 2, 30, footer, COLOR_SECONDARY);
    }
